...
class RUG extends React.Component {
    constructor({initialState, ssrSupport}) {
        ...
        this.state = {
            images: this.initImages(initialState),
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.initialState != this.props.initialState) {
            this.state = {
                images: this.initImages(nextProps.initialState)
            }
        }
    }

    initImages(initialState) {
        return initialState.reverse().map((item) => {
            return this.create({
                done: true,
                ...item
            });
    ...
    onSelected(uid) {
        this.setState({
            images: this.state.images.map((item) => Object.assign({}, item, {
    ...
    async uploadFiles(files) {
        ...
        this.setState({
            ...
            if (this.props.autoUpload) { // wait for setState process to finish
                let i = -1;
     ...
     */
        ...
        if (!isAccepted(file.type, accept.map((type) => `${acceptType}/${type}`))) {
            warning("accept");
        }
            ...
            <Context.Provider value={contextValue}>
                <div className={
                    ...
                    <input multiple type="file"
                        ...
                        onChange={
                            (event) => this.uploadFiles(event.target.files)
                        }/>
                </div>
            </Context.Provider>
        );
